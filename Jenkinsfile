  ...
  try {
	  ...
	  docker.withRegistry('http://secureci:8182','docker') {
		  ...
		  stage ("TestSetup"){
			 ...
			 try { 
				 mysqlContainer = docker.image("secureci:8182/mysql:latest").run('-p 3306 --net=\"host\"','/start.sh')
				 ...
				 tomcatContainer = docker.image("secureci:8182/tomcat:latest").run('-p 8080 -v ${WORKSPACE}/target:/home/tomcat/tmp --net=\"host\"','/bin/cat')
