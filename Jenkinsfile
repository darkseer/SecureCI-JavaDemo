	...
	docker.withRegistry('http://secureci:8182','docker') {
		...
		try { 
			...
			IntegrationTest: {
			    try {
				wrap([$class: 'Xvfb']) {
				    sh "mvn -Dwebdriver.chrome.driver=/usr/java/secureci-testing-framework-1.3.0/chromedriver -Dwebdriver.gecko.driver=/usr/local/bin/geckodriver -Dmaven.test.failure.ignore=false verify -Dtomcat.port=${TOMCATPORT} -Dtomcat.ip=${DOCKER_HOST_INTERNAL_IP} -X sonar"
				}
			    }
			    catch (err){
